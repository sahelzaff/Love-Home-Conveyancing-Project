...
const CustomSelect = ({ options, placeholder }) => {
  ...
  return (
    <div className="relative w-64 z-50">
      <div
        className="relative w-full py-3 pl-4 pr-8 text-center bg-[rgb(248,248,255)] text-black border border-2 border-white rounded leading-tight focus:outline-none custom-select cursor-pointer"
        onClick={toggleDropdown}
      >
        {selectedOption || placeholder}
        <div className="absolute right-0 top-0 h-full flex items-center bg-orange-500" style={{ backgroundColor: '#0077B6', width: '2.5rem' }}>
          <svg
      ...
      {isOpen && (
        <ul className="absolute z-50 w-full bg-white text-black border border-white rounded mt-1 bottom-full mb-1">
          {options.map((option) => (
            <li
              key={option}
...
const Hero = () => {
  return (
    <div className="relative w-full h-screen bg-cover bg-center overflow-hidden" style={{ backgroundImage: `url(${assets.background})` }}>
      <div className="absolute inset-0 flex flex-col items-center md:items-start justify-center text-white px-6 md:px-5 pb-20 z-40 xl:pt-28 max-w-screen-xl mx-auto">
        <h1 className="text-xl md:text-[80px] font-bold text-center md:text-left font-foundersGrotesk w-full">
          Simplify Your Move
        </h1>
        <div className='relative w-full mt-4'>
          <p className="text-lg md:text-2xl px-6 pt-6 text-start tracking-widest font-poppins font-semibold text-white w-full md:w-auto">
            Get a Tailored Conveyancing Quote in Minutes!
          </p>
        </div>
        <div className='relative w-full mt-24'>
          <p className="text-lg md:text-2xl  pt-3 text-left font-poppins w-full md:w-auto font-medium">Property Conveyancing</p>
        </div>
